...
self.addEventListener('install', (e) => {
    const static = caches.open(STATIC_CACHE).then(cache => cache.addAll(APP_SHELL).catch(error => console.error('Error caching static assets:', error)));
    const inmutable = caches.open(INMUTABLE_CACHE).then(cache => cache.addAll(APP_SHELL_INMUTABLE).catch(error => console.error('Error caching inmutable assets:', error)));
    e.waitUntil(Promise.all([static, inmutable]));
});
